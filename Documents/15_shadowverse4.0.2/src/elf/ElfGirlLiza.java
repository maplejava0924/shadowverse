...
public class ElfGirlLiza extends Follower{
	...
	public void fanfare(Leader turnLeader,Leader noTurnLeader, int choiceNumber) {
		...
		for(int i=0;i<turnLeader.getFieldCount();i++) {
			turnLeader.getFieldList()[i].setLiza(true);
		}
		...
		TurnEndAddEffect turnEndAddEffect = (p1,p2) ->{
			for(int i=0;i<p2.getFieldCount();i++) {
				if(p2.getFieldList()[i].isLiza()==true) {  //リザ(リザの効果を外す)
					System.out.println(name+"のターン終了時リーダー付与効果発動！");
					p2.getFieldList()[i].setLiza(false);  //「相手のターン終了時まで」なのでp2で良い
				}
			}
		};
