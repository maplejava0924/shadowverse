...
public class BigKnuckleBodyguard extends Follower{

	public BigKnuckleBodyguard() {
		name="豪拳の用心棒";
		rank="vampire";
		ap=3;
		hp=4;
	...
	public boolean canPlayChoice(Leader turnLeader, Leader noTurnLeader, int choiceNumber) {
		...
		if (choiceNumber > noTurnLeader.getFieldCount() - 1) {
			System.out.println("フィールド枚数は" + noTurnLeader.getFieldCount() + "枚です。もう一度入力して下さい。");
			result = false;
			...
			result = false;
		}else if(noTurnLeader.getFieldList()[choiceNumber].getClas()!='f'||noTurnLeader.getFieldList()[choiceNumber].getHp()>3) {  //&&→||
