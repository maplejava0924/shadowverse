...
public class Card implements Cloneable {
	...
	//PPの最大値
	public static final int MAX_PP = 10;
	
	//フィールドにおけるカードの最大値
	public static final int MAX_FIELD = 5;
	
	//ハンドに持てるカードの最大値
	public static final int MAX_HAND = 9;
	
	//復讐状態になる体力の境界値
	public static final int REVENGE_LINE = 10;
	...
	//freeEvolve無料進化か否か
	public void evolve(Leader turnLeader, Leader noTurnLeader, int evolveNumber, int choiceNumber,boolean freeEvolved) {

	}

	// 効果によるダメージメソッド
	public void effectAttack(List<Card> Field, int a, int b) { // Fieldサイドのa番目にb点与える。

		if (!Field.get(a).isLiza()) { // リザの効果を受けてないなら、b点与える
			Field.get(a).setHp(Field.get(a).getHp() - b);
		}
	...
	// 場に出たときの効果（ラプンツェルなど）「場に出たとき」
	public void fieldSetEffect(Leader turnLeader, Leader noTurnLeader) {
	}
	...
	public void playOperation(Leader turnLeader, Leader noTurnLeader, int playNumber) {
		...
		// playしたplayNumberの手札をリストから削除する。
		//Listの機能で勝手に間はつめられる。
		//ここをthisで指定できたらplayNumberの引数がいらなくなる。
		turnLeader.getHandList().remove(playNumber);
	...
	public void fieldSet(Leader turnLeader, Leader noTurnLeader, Card card) {

		// フィールドにcardをセット。
		turnLeader.getFieldList().add(card);
		// フィールド番号をセット。
		turnLeader.getFieldList().get(turnLeader.getFieldList().size()-1).setfN(turnLeader.getFieldList().size());
		
		// 場に出た時の効果
		this.fieldSetEffect(turnLeader, noTurnLeader);
